...
export default function WithdrawFormDialog(prop: {
  open: boolean;
  handleClose: () => void;
  data: TokenData;
  tab: string;
  ...
  const { state, send } = useContractFunction(
    NFTManagerContract,
    tab === "aave" ? "useAave" : "withdrawFromLiquidityPool",
    {
      transactionName:
        tab === "aave" ? "Supplied in aave" : "Withdrawn from wallet",
    }
  );
  const { status } = state;
  ...
  const handleWithdraw = () => {
    setLoading(true);
    const aaveWithdrawCallData = aaveInteractor(
      "withdraw",
    ...
    tab === "aave"
      ? send(tokenId, aaveWithdrawCallData).then((val) => {
          setTxn(val?.status);
          setLoading(false);
  ...
  // @todo add token Decimals here with token address
